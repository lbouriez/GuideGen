...
export function generateClaudeMd(
  ...
  guidelinesSection += 'Complete coding guidelines are available in `.guidelines/`:\n\n';

  // List by domain
  for (const [domain, domainGuidelines] of domainMap) {
    if (domainGuidelines.length > 0) {
      guidelinesSection += `\n**${domain.charAt(0).toUpperCase() + domain.slice(1)}**:\n`;
      for (const g of domainGuidelines) {
        guidelinesSection += `- [\`.guidelines/${domain}/${g.fileName}\`](../.guidelines/${domain}/${g.fileName}) - ${g.type}\n`;
      }
    }
  }
